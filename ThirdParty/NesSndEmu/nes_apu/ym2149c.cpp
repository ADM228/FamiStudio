...
void Ym2149c::SetMask(uint8_t mask){
	m_channelMask = mask;
}
...
uint16_t Ym2149c::Tick()
{
	...
	for (int v = 0; v < 3; v++)
	{
		...
		if (m_toneCounter[v] >= m_tonePeriod[v])
		{
			m_toneEdges ^= 0xf<<(v*4);
			m_toneCounter[v] = 0;
			if (m_toneEdges & 0xf<<(v*4)) m_triggerMask |= (1<<v);	// Trigger on rising edge
		}
	}
...
int16_t Ym2149c::ComputeNextSample(uint32_t* pSampleDebugInfo)
{
	...
	int16_t out = (				// DC adjusting removed
		(m_channelMask & 1<<0 ? levelA : 0) +
		(m_channelMask & 1<<1 ? levelB : 0) + 
		(m_channelMask & 1<<2 ? levelC : 0)) / 2;
	if (pSampleDebugInfo)
